...
## House rules for this repo — read before using any result

Vendored from `nextlevelbuilder/ui-ux-pro-max-skill` (MIT, see `LICENSE` in this
folder). Kept unmodified except for this section and the script paths below.

**It never picks Otis's colors or fonts.** All in 1 Events and Meridian both have
locked tokens in `meridian-stack` / `references/design-tokens.md`. On either of
those brands, ignore this skill's palette and font-pairing output entirely and
use it only for layout, interaction, accessibility and stack guidance. Its
colour and type database is for **client projects with no brand yet**, and for
concept work.

**It overlaps with skills already here.** `awesome-design` holds the craft
principles, `taste` holds Otis's own calls, `apple-interface` holds app-surface
rules, `cinematic-web` holds hero and landing-page craft. Where they disagree,
those four win — they were written for his work; this was not.

**Running the script.** Paths below are repo-relative and work in a session
rooted at this repo. If that path does not resolve because the skill is loaded
from the installed plugin instead, use
`${CLAUDE_PLUGIN_ROOT}/skills/ui-ux-pro-max/scripts/search.py`. Needs Python 3
(present here: 3.11). The script makes no network calls and writes nothing
