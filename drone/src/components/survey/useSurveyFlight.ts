...
export function useSurveyFlight(sim: Sim, link: Link, plan: SurveyPlan) {
  ...
  const [up, setUp] = useState<{ state: 'IDLE' | 'UPLOADING' | 'READY' | 'FAILED'; key: string; msg: string; fence: 'OFF' | 'ON' | 'UNCONFIRMED' | 'FAILED'; resume: ResumePoint | null; kind: 'NEW' | 'RESUME' | 'REFLY'; flown: boolean }>({ state: 'IDLE', key: '', msg: '', fence: 'OFF', resume: null, kind: 'NEW', flown: false });
  ...
  const upload = useCallback(async (from: ResumePoint | null = null, reflyLines: FlightLine[] | null = null) => {
    ...
    setUp({ state: 'UPLOADING', key, msg: '', fence: 'OFF', resume: from, kind, flown: false });
    let fenceState: 'OFF' | 'ON' | 'UNCONFIRMED' | 'FAILED' = 'OFF';
    try {
      if (useFence) {
        try { fenceState = (await link.uploadFence(fenceItems(fence, site.origin))).enabled ? 'ON' : 'UNCONFIRMED'; }
        catch { fenceState = 'FAILED'; }
      }
      await link.uploadMission(m.items, false);
      sim.setLiveMission(m.roles, link.missionSeqOffset, kind, reflyLines ?? undefined);
      if (from) sim.clearLiveResume();
      setUp({ state: 'READY', key, msg: `${m.items.length} items on the aircraft`, fence: fenceState, resume: from, kind, flown: false });
    } catch (e) {
      setUp({ state: 'FAILED', key, msg: e instanceof Error ? e.message : String(e), fence: fenceState, resume: from, kind, flown: false });
    }
  }, [plan, site, ap, mission, key, useFence, fence, link, sim]);

  const begin = useCallback(async () => {
    setStart({ state: 'STARTING', msg: [] });
    const r = await link.startMission();
  ...
  /** Uploaded and not yet flown: ready for Start. */
  const armable = uploaded && !up.flown;
  const onGround = !t.armed || t.altRelM < 1;
  return {
    checks, gateOk, fence: useFence ? fence : null, useFence, setUseFence, mission, count,
    upload: up, uploaded, armable, start, onGround,
    uploadMission: () => upload(null), uploadResume: () => sim.liveResume && upload(sim.liveResume),
